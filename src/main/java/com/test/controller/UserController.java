...
public class UserController {
	...
	@RequestMapping("/socketTest") //功能测试页面（新版本软件没有该模块，需添加功能测试页面）
	...
	@RequestMapping("/turnHeartBeat") // 开关心跳包，功能测试页面（新版本软件没有该模块，需添加功能测试页面）
...
*/
	...
	public String login(HttpServletRequest request, Model model) {
		...
		if (username != null && password != null) {
			...
			if (user != null && username.equals(user.getUsername()) && password.equals(user.getPassword())) {
				...
				return "/jsp/index1.jsp";//页面重构(LAYUI)后登录后进入的首页	
				//return "/jsp/index.jsp";//原版(VUE)登录后进入的首页
			} else {
				DataObject data = new DataObject();
				//.判断当前语言环境，返回对应的登录界面
				...
				if (ds.getDevice().getDevMac() != null && ds.getDevice().getDevMac().equals(devMac)) {// 找到设备对应的scoket线程
					...
					if (data.getError().equals("The device is offline or you haven't added the device yet!")) {// 如果没有，直接发送新指令
						...
						try {
							...
							//记录用户指令
							UserCmdMessage userCmdMessage = new UserCmdMessage();
						    Date nowTime = new Date();//获取当前时间
							userCmdMessage.setDate(nowTime);
							userCmdMessage.setCmdType(userCmdMessage.CMD_OPEN_FIND_ZIGBEE);
							userCmdMessage.setDevMac(ds.getDevice().getDevMac());
							userCmdMessage.setUserid(ds.getDevice().getUserid());
							userCmdMessage.setCmdData(socketcmd.getCmdString());
							userCmdMessageDao.insert(userCmdMessage);
							break;
						} catch (IOException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
	...
	public void removeZigbee(HttpServletRequest request, HttpServletResponse response) {
		...
		if (user != null && zigbeeMac != null) {
			...
			data.setError("Parameter error!");// data.setError("参数错误!");
		}
	...
	@RequestMapping("/switchByDev") // 控制集控器下所有zigbee节点开关 +
	// 1,接受集控器的mac地址或短地址
	...
	public void switchByDev(HttpServletRequest request, HttpServletResponse response) {
		...
		DataObject data = new DataObject();
		data.setError("The device is offline or you haven't added the device yet!");// data.setError("该设备离线或您尚未添加该设备!");
		if (SocketTool.socketList != null) {
			for (DeviceSocket ds : SocketTool.socketList) {// 获取socket列表内是否有该设备
				if (ds.getDevice().getDevMac() != null && ds.getDevice().getDevMac().equals(devMac)) {// 找到设备对应的scoket线程
					SocketCmd socketcmd = new SocketCmd();
					...
					if (data.getError().equals("The device is offline or you haven't added the device yet!")) {
						...
						try {
							...
							userCmdMessage.setUserid(ds.getDevice().getUserid());
							userCmdMessage.setCmdData(socketcmd.getCmdString());
							if (cmd.equals("打开")) {//广播开灯
								userCmdMessage.setCmdType(userCmdMessage.CMD_BROADCAST_ON);
							} else if (cmd.equals("关闭")) {//广播关灯
								userCmdMessage.setCmdType(userCmdMessage.CMD_BROADCAST_OFF);
							}
							//指令记录数据库
							userCmdMessageDao.insert(userCmdMessage);
							break;
				...
				if (ds.getDevice().getDevMac() != null && ds.getDevice().getDevMac().equals(devMac)) {// 找到设备对应的scoket线程
					...
					if (data.getError().equals("The device is offline or you haven't added the device yet!")) {
						...
						try {
						    ...
						    Date nowTime = new Date();//获取当前时间
							userCmdMessage.setDate(nowTime);
							userCmdMessage.setCmdType(userCmdMessage.CMD_BROADCAST_DIM);
							userCmdMessage.setDevMac(ds.getDevice().getDevMac());
							userCmdMessage.setUserid(ds.getDevice().getUserid());
							userCmdMessage.setCmdData(socketcmd.getCmdString());
							userCmdMessageDao.insert(userCmdMessage);//插入指令记录到数据库
							break;
						} catch (IOException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
					...
					if (ds.getDevice().getDevMac() != null && ds.getDevice().getDevMac().equals(zb.getDevMac())) {// 找到设备对应的scoket线程
						...
						if (data.getError().equals("Unable to connect to the controller where the node is located!")) {
							...
							try {
							    ...
							    Date nowTime = new Date();//获取当前时间
								userCmdMessage.setDate(nowTime);
								userCmdMessage.setCmdType(userCmdMessage.CMD_ZIGBEE_DIM);
								userCmdMessage.setDevMac(ds.getDevice().getDevMac());
								userCmdMessage.setUserid(ds.getDevice().getUserid());
								userCmdMessage.setCmdData(socketcmd.getCmdString());
								userCmdMessageDao.insert(userCmdMessage);//插入指令记录到数据库
					...
					if (ds.getDevice().getDevMac() != null && ds.getDevice().getDevMac().equals(zb.getDevMac())) {// 找到设备对应的scoket线程
						...
						if (data.getError().equals("Unable to connect to the controller where the node is located!")) {
							...
							try {
								...
								userCmdMessage.setCmdData(socketcmd.getCmdString());
								if (cmd.equals("打开")) {
									userCmdMessage.setCmdType(userCmdMessage.CMD_ZIGBEE_ON);
								} else if (cmd.equals("关闭")) {
									userCmdMessage.setCmdType(userCmdMessage.CMD_ZIGBEE_OFF);
								}
								userCmdMessageDao.insert(userCmdMessage);//插入指令记录到数据库
								break;
							} catch (IOException e) {
	...
	@RequestMapping("/getUseZigbeeMessage") 
	// 向控制组中添加节点，先查询集控器和可用的节点信息，返回js中，利用信息进行数据选择
	...
	public void addZigbeeToGroup(HttpServletRequest request, HttpServletResponse response) {
		...
		if (group != null) {
						...
						if (ds.getDevice().getDevMac() != null && zb.getDevMac().equals(ds.getDevice().getDevMac())
							...
							if (data.getError().equals("Unable to connect offline controller!")) {
								...
								try {
								    ...
								    Date nowTime = new Date();//获取当前时间
									userCmdMessage.setDate(nowTime);
									userCmdMessage.setCmdType(userCmdMessage.CMD_ZIGBEE_BIND_GROUP);
									userCmdMessage.setDevMac(ds.getDevice().getDevMac());
									userCmdMessage.setUserid(ds.getDevice().getUserid());
									userCmdMessage.setCmdData(socketcmd.getCmdString());
									userCmdMessageDao.insert(userCmdMessage);//插入指令记录到数据库
			...
			data.setError("Group does not exist!");//data.setError("分组不存在！");
		}
	...
	@RequestMapping("/refreshGroupMsgOfZigbee")//好像没有用到
					...
					if (ds.getDevice().getDevMac() != null && zb.getDevMac().equals(ds.getDevice().getDevMac())
						...
						if (data.getError().equals("Unable to connect offline controller!")) {
							...
							try {
							    ...
							    Date nowTime = new Date();//获取当前时间
								userCmdMessage.setDate(nowTime);
								userCmdMessage.setCmdType(userCmdMessage.CMD_ZIGBEE_DELETE_GROUP);
								userCmdMessage.setDevMac(ds.getDevice().getDevMac());
								userCmdMessage.setUserid(ds.getDevice().getUserid());
								userCmdMessage.setCmdData(socketcmd.getCmdString());
								userCmdMessageDao.insert(userCmdMessage);//插入指令记录到数据库
					...
					if (ds.getDevice().getDevMac() != null && ds.getDevice().getUserid().equals(group.getUserid())) {// 集控器用户id与组用户id要一致
						...
						if (data.getError().equals("Unable to connect offline controller!")) {
							...
							try {
								...
								userCmdMessage.setCmdData(socketcmd.getCmdString());
								if (cmd.equals("打开")) {
									userCmdMessage.setCmdType(userCmdMessage.CMD_GROUP_ON);
								} else if (cmd.equals("关闭")) {
									userCmdMessage.setCmdType(userCmdMessage.CMD_GROUP_OFF);
								}
								userCmdMessageDao.insert(userCmdMessage);//插入指令记录到数据库
					...
					if (ds.getDevice().getUserid().equals(group.getUserid())) {// 找到设备对应的scoket线程
						...
						if (data.getError().equals("Unable to connect offline controller!")) {
							...
							try {
							    ...
							    Date nowTime = new Date();//获取当前时间
								userCmdMessage.setDate(nowTime);
								userCmdMessage.setCmdType(userCmdMessage.CMD_GROUP_DIM);
								userCmdMessage.setDevMac(ds.getDevice().getDevMac());
								userCmdMessage.setUserid(ds.getDevice().getUserid());
								userCmdMessage.setCmdData(socketcmd.getCmdString());
								userCmdMessageDao.insert(userCmdMessage);//插入指令记录到数据库
	...
	@RequestMapping("/ployRefresh") // 刷新，VUE界面，新界面没用到
	...
	@RequestMapping("/savePloyEdit") // 保存编辑，VUE界面用到，新界面没用到
