...
//		times = currentTime.getTime() - ((long)p.getTimeZone() * 60 * 1000);// 获取时区偏移之后的时间
	...
	/**
	 * 根据operate和ploy编辑指令通过ds.socket发送
	 * @param ds
	 * @param operate
	 * @param ploy
	 */
	private void sendCmd(DeviceSocket ds, PloyOperate operate, Ploy ploy) {
		sc = new SocketCmd();
		...
		userCmdMessage.setCmdType(userCmdMessage.CMD_PLOY_TIMING);
		userCmdMessage.setCmdData(sc.getCmdString());
		userCmdMessage.setDate(nowDate);
		userCmdMessage.setDevMac(ds.getDevice().getDevMac());
		userCmdMessage.setUserid(ds.getDevice().getUserid());
		SocketTool.InsertPloyTimingCmdToUserCmdMessage(userCmdMessage);
